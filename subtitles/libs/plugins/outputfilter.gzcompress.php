...
* ####    $smarty_object->load_filter('output','gzcompress');
* from application.
* -------------------------------------------------------------
*/
function smarty_outputfilter_gzcompress($tpl_source, &$smarty)
{
	...
	The trade off between CPU usage and compression isn't worth going above level 1 or 2 in most cases.
	9 will not compress most pages more than a few bytes better than levels 1 & 2, but server load dramatically increases.
	-------------------------------------
	*/
	$compression_level = 9;
	...
	*/
	...
	&& (ereg("gzip", $_SERVER["HTTP_ACCEPT_ENCODING"]) || $force_compression))
	{
